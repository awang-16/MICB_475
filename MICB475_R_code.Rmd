...
function: performs analyses on microbiome data for alpha+beta diversity, differential expression (taxonomic+functional)
---
...
```
## Using DESeq2 data to produce barplots + volcano plots (smoking) (Manuscript figure 3) @IMOGEN EDIT
```{r}
...
```
## Using DESeq2 data to produce barplots + volcano plots (NONsmoking) (Supplemental Figure 3) @IMOGEN EDIT
```{r}
...
vol_plot_nonsmok <- res_nonsmoke %>%
  ...
  labs(x = "Log2 Fold Change", y = "-Log10 (padj)")
vol_plot
# ggsave(filename="figures/vol_plot_nonsmokingLDL.png",vol_plot_nonsmok)
```
...
ggsave(filename="figures/picrust_PCA_LDL_all",PCA_LDL_all)
ggsave(filename="figures/picrust_PCA_nonsmok_LDL",PCA_nonsmok_LDL)
ggsave(filename="figures/picrust_PCA_smok_LDL",PCA_smok_LDL)
